...
This crate can also be used as a Rust library (in addition to Python bindings).

### Add as Dependency

```toml
[dependencies]
docxtplrs = { git = "https://github.com/yourusername/docxtplrs" }
```
...
```rust
use docxtplrs::DocxTemplate;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Load template
    let doc = DocxTemplate::new("template.docx".to_string())?;
    
    // Note: The current implementation is primarily designed for Python.
    // The render() method expects Python dictionary types.
    // For pure Rust usage, you may need to adapt the code or use
    // alternative crates like `docx-rs`.
    
    Ok(())
}
```

### Current Limitations for Rust Users

The current codebase is optimized for Python interop:

| Feature | Python | Rust (Native) |
|---------|--------|---------------|
| Template rendering | ✅ Full | ⚠️ Requires adaptation |
| RichText | ✅ Full | ⚠️ Requires adaptation |
| InlineImage | ✅ Full | ⚠️ Requires adaptation |
| Custom Jinja filters | ✅ Full | ❌ Python-only |
| Type hints | ✅ Full | ⚠️ Limited |

For pure Rust docx generation, consider these alternatives:
- [`docx-rs`](https://github.com/bokuweb/docx-rs) - Feature-rich pure Rust docx library
- [`docx`](https://crates.io/crates/docx) - Another Rust docx library
